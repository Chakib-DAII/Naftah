...
public class NaftahParserHelper {
    ...
    public static void prepareDeclaredFunction(org.daiitech.naftah.core.parser.NaftahParserBaseVisitor naftahParserBaseVisitor,
                                               DeclaredFunction function) {
        if (function.getParameters() == null && hasChild(function.getParametersContext()))
            function.setParameters((List<DeclaredParameter>) visit(naftahParserBaseVisitor, function.getParametersContext()));
    ...
    public static Map<String, Object> prepareDeclaredFunctionArguments(org.daiitech.naftah.core.parser.NaftahParserBaseVisitor naftahParserBaseVisitor,
                                                                       List<DeclaredParameter> parameters, List<Pair<String, Object>> arguments) {
        if (parameters.size() < arguments.size())
            throw new RuntimeException("Too many arguments");

        // how many params don't have defaults
        List<DeclaredParameter> requiredParams = parameters.stream()
                .filter(p -> p.getDefaultValue() == null)
                .toList();

        // arguments that have names
        Map<Integer, Pair<String, Object>> namedArguments = IntStream.range(0, arguments.size())
                .mapToObj(i -> {
        ...
        if (namedArguments.isEmpty()) {
            ...
            for (var entry : positionalArguments.entrySet()) {
                ...
                if (namedArguments.containsKey(entry.getKey())) {
                    throw new RuntimeException("Argument '" + paramName + "' specified both positionally and by name");
                }
