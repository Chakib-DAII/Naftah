...
public final class RuntimeClassScanner1 {
  ...
  public static final Set<String> IGNORE_CLASS =
      IGNORE.stream().map(s -> s + CLASS_EXTENSION).collect(Collectors.toSet());
  ...
  public static Class<?> loadClass(String className, URLClassLoader classLoader) {
    var loaders =
        Arrays.copyOf(CLASS_LOADERS, CLASS_LOADERS.length + (Objects.nonNull(classLoader) ? 0 : 1));
  ...
  public static Map<String, ScannedClass> findClassesInDirectory(File root, File dir) {
    ...
    for (File file : Objects.requireNonNull(dir.listFiles())) {
      if (file.isDirectory()) {
      ...
      } else if (IGNORE_CLASS.stream().noneMatch(s -> file.getName().endsWith(s))
          && file.getName().endsWith(CLASS_EXTENSION)) {
        String className =
        ...
        if (Objects.nonNull(clazz)) {
          ...
          scannedClassOptional.ifPresent(
              scannedClass -> classes.put(scannedClass.qualifiedName(), scannedClass));
          ;
        }
      } else if (file.getName().endsWith(JAR_EXTENSION)
          || file.getName().endsWith(JMOD_EXTENSION)) {
          ...
          try (URLClassLoader loader =
            ...
            classes.putAll(
                findClassesInJar(tempInnerJar).keySet().stream()
                    .map(
                        className ->
                            Optional.ofNullable(loadClass(className, loader))
                                .map(
                                    clazz -> {
                                      var scannedClassOptional = ScannedClass.safeOf(clazz);
                                      return scannedClassOptional
                                          .map(
                                              scannedClass ->
                                                  Map.entry(
                                                      scannedClass.qualifiedName(), scannedClass))
                                          .orElse(null);
                                    })
                                .orElse(null))
   ...
   */
  public static File jarEntryToTempFile(JarFile outerJar, JarEntry innerJarEntry)
      throws IOException {
    // Create a temp file
    File tempInnerJar =
        File.createTempFile(
